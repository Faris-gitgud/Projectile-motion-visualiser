...
"""
the functioning of the simulator using physics equations of projectile
and basic math
"""

def ideal_trajectory(v0, angle_degrees, g=9.81, num_points=300):
    """
    Calculate the projectile trajectory (ideal case without air resistance)
    """
    ...
    #3. position coordinates at time 't'
    x = v0 * np.cos(theta) * t
...
"""
graphical display of the projectile simulator

"""

#1.area that the plot occupies
...
#7. limits of x and y axes enabling the grid and also the legend of plot
ax.set_xlim(0, 100)
ax.set_ylim(0, 50)
ax.grid(True, linestyle='--', alpha=0.6)
ax.legend(loc='upper right')

#8. making a result box which displays the results of each launch
...
#9. update the result box with the initial results
update_results(time_flight, max_range, max_height)

"""
]making interactive sliders to adjust inintial conditions before launch.
"""

#1. positioning the sliders on the plot
ax_v0 = plt.axes([0.1, 0.2, 0.8, 0.03])
ax_angle = plt.axes([0.1, 0.15, 0.8, 0.03])

#2. creating the sliders
slider_v0 = Slider(ax_v0, 'Initial Velocity (m/s)', 5, 50, valinit=v0_initial)
slider_angle = Slider(ax_angle, 'Launch Angle (degrees)', 0, 90, valinit=angle_initial)
...
def update(values):
    ...
    # 3.Recalculate trajectory with new values given by user on the sliders
    x, y, time_flight, max_range, max_height = ideal_trajectory(v0, angle)

    #4. Update the trajectory line and peak point marker accordint to new values
    trajectory_line.set_data(x, y)
    peak_point.set_data([max_range / 2], [max_height])

    # 5. rescale the axes limits based on new trajectory
    ax.set_xlim(0, max_range + 5)
    ax.set_ylim(0, max_height + 5)

    #6. Update the result box with new results
    update_results(time_flight, max_range, max_height)
    fig.canvas.draw_idle()

#7. slider event listeners to call the update function when slider values change    
slider_v0.on_changed(update)
slider_angle.on_changed(update)

#8. show the plot with the interactive sliders
plt.show()
